...
void loop() {
  if (first) {
    ...
    goToAngle(potToAngle(470), 30.0, 3.0);
    waitForButton(START_BUTTON);
    goToAngle(0.0, 30.0, 3.0);
    first = false;
  }
  ...
  if (cur_triggered = BOTH) {
    goToAngle(potToAngle(470), 30.0, 3.0); // Arm down = -0.8 rad; 300 analogRead.
    writeMotors(60, 60);
    delay(200);
  }
...
void goToAngle(double angle/*radians*/, double kP, double kI, double kD) {
  ...
  double sum = 0.0;
  while (abs(error) > 0.1 || abs(prev_error - error) > 0.001) {
